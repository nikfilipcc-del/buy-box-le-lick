...
export class SkioPlanPicker extends LitElement {
  ...
  static styles = [
    css`
      ...
      .group-container--selected .per-box {
        font-size: 24px;
    ...
    return html`
      <div class="skio-radio__container">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle class="skio-radio" cx="12" cy="12" r="8" fill="currentColor"></circle>
        </svg>
      </div>
    `
  }
                  ...
                  <div class="group-topline">
                    ...
                    <div class="skio-price" aria-live="polite">
                      ${this.options?.show_compare_price && this.selectedVariant?.compare_at_price > this.selectedVariant.price
                        ? html`<s class="skio-price-compare--amount aria-hidden="true">${this.money(this.selectedVariant.compare_at_price).replace('.00', '')}</s>`
                        : ''}
                      <span class="skio-price--amount" data-base-price="${(this.money(this.selectedVariant.price)).replace('.00', '')}">${(this.money(this.selectedVariant.price)).replace('.00', '')}</span>
                      <p class="savings ${this.options?.discount_style} per-box">/ Box</p> 
                    </div>
                  </div>
                </label>
              </div>
                    ...
                    <div class="group-topline">
                      ...
                      <div class="skio-price" aria-live="polite">
                        ...
                        (this.selectedVariant?.compare_at_price > this.selectedVariant.price || this.selectedVariant.price > this.price(group.selected_selling_plan, false))
                          ? html`<s class="skio-price-compare--amount" aria-hidden="true">${this.money(this.selectedVariant.compare_at_price ? this.selectedVariant.compare_at_price : this.selectedVariant.price).replace('.00', '')}</s>`
                          : ''}
                        <span class="skio-price--amount" data-base-price="${this.price(group.selected_selling_plan).replace('.00', '')}">${this.price(group.selected_selling_plan).replace('.00', '')}</span>                  
 ...
 const isSubscribe = this.selectedSellingPlanGroup != null;
 console.log('isSubscribe==>', this.selectedSellingPlanGroup)

  document.querySelectorAll('.one-time').forEach(el => {
    el.style.display = isSubscribe ? 'none' : 'block';
  });

  document.querySelectorAll('.subscribe').forEach(el => {
    el.style.display = isSubscribe ? 'block' : 'none';
  });
}
